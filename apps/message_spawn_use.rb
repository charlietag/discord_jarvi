...
bot.message(with_text: /(?:(?!^\/(#{$skip_commands.join('|')})))(?:(?!#{newbie_fail}).)*(#{newbie_ans})+(?:(?!#{newbie_fail}).)*/i) do |event|
...
bot.message(with_text: /(?:(?!^\/(#{$skip_commands.join('|')})))(.*(#{newbie_fail})+.*){1,}/i) do |event|
